package com.enigma.services.impl;

import com.enigma.entities.Transaction;
import com.enigma.repositories.TransactionRepository;
import com.enigma.services.*;
import org.junit.jupiter.api.Test;
import org.junit.jupiter.api.extension.ExtendWith;
import org.mockito.Mockito;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.boot.test.context.SpringBootTest;
import org.springframework.boot.test.mock.mockito.MockBean;
import org.springframework.boot.test.mock.mockito.SpyBean;
import org.springframework.test.context.junit.jupiter.SpringExtension;

import static org.junit.jupiter.api.Assertions.*;

@ExtendWith(SpringExtension.class)
@SpringBootTest
class TransactionServiceImplTest {

    @MockBean
    TransactionRepository transactionRepository;

    @MockBean
    UserService userService;

    @MockBean
    ServicesService servicesService;

    @MockBean
    ItemService itemService;

    @MockBean
    WalletService walletService;

    @SpyBean
    TransactionService transactionService;

    @Test
    void findAll_should_call_transactionRepository_findAll_once() {
        transactionService.findAll();
        Mockito.verify(transactionRepository, Mockito.times(1)).findAll();
    }

    @Test
    void saveTransaction_should_call_transactionRepository_save_once() {
        transactionService.saveTransaction(new Transaction());
        Mockito.verify(transactionRepository, Mockito.times(1)).save(new Transaction());
    }

    @Test
    void getTotalPrice() {
    }

    @Test
    void findTransactionById() {
    }

    @Test
    void updateTransaction() {
    }

    @Test
    void getChangeTransaction() {
    }

    @Test
    void deleteTransactionById() {
    }
}
